...
| Pane-first roots and ordered secondary tabs | [HierarchySnapshotTests](../CMUXMaestroPreviewTests/HierarchySnapshotTests.swift): `paneOutlineMatchesSplitRootsAndSecondaryTabsInsteadOfAgentOwnership`, `paneSnapshotsReplaceMovesReordersAndClosedAnchorsWithoutStalePlacement`, `missingDeniedAndInvalidPaneDataNeverInventGrouping`; [SidebarLayoutRenderingTests](../CMUXMaestroPreviewTests/SidebarLayoutRenderingTests.swift): `paneTabsRenderUnderTheirOwnRootAndCollapseIndependentlyOfAgentOwnership` | Authoritative optional SDK pane membership replaces orchestration parentage in Workspaces. Synthetic pixel bounds verify Developer 2 is indented 12 points beneath Developer 1, while other panes remain siblings, at 240 and 340 points. Collapse hides only that pane's secondary tabs. Missing/invalid data remains explicitly ungrouped. Hidden first-tab results stay hidden while a plain tab anchors visible siblings. This offscreen proof is not installed-host acceptance. |
