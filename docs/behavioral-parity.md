...
| Explicit terminal-backed orchestration | [test-cmux-maestro-orchestrator.py](../scripts/test-cmux-maestro-orchestrator.py): concurrent startup, background-tab, exact ownership/resume, verified follow-up boundary, live-resource cap/reclamation, archive/recovery, automatic exit and focus contracts; [SidebarOrchestrationTests](../CMUXMaestroPreviewTests/SidebarOrchestrationTests.swift): validated ancestry/lifecycle/timestamps, stale qualification and privacy-preserving current-window filtering; [test-copilot-sandbox.sh](../scripts/test-copilot-sandbox.sh): observer-readable/private-sibling-denied proof | The controller and skill are bundled and production setup installs them. Live CMUX spawn/follow-up acceptance is operator-gated and remains unverified in this implementation worktree. |
